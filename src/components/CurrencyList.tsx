...
const CurrencyList: React.FC = () => {
  const currencies = [
    { sr: 1, country: 'Pakistan', currency: 'PKR', symbol: 'Rs.' },
  ];
          ...
          <tbody className="text-gray-700 text-sm">
            {currencies.map((currency, index) => (
              <tr key={index} className="border-b border-gray-300">
                <td className="py-2 px-4">{currency.sr}</td>
                <td className="py-2 px-4">{currency.country}</td>
                <td className="py-2 px-4">{currency.currency}</td>
