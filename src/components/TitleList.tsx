...
const TitleList: React.FC = () => {
  const titles = [
    { sr: 1, type: 'Relation Title', title: 'W/O', gender: 'Female', seqNo: 0 },
    { sr: 2, type: 'Person Title', title: 'Mst.', gender: 'Female', seqNo: 0 },
    { sr: 3, type: 'Relation Title', title: 'D/O', gender: 'Female', seqNo: 0 },
  ];
  ...
  return (
    <div className="p-4">
          ...
          <tbody className="text-gray-700 text-sm">
            {titles.map((title, index) => (
              <tr key={index} className="border-b border-gray-300">
                <td className="py-2 px-4">{title.sr}</td>
                <td className="py-2 px-4">{title.type}</td>
                <td className="py-2 px-4">{title.title}</td>
                <td className="py-2 px-4">{title.gender}</td>
                <td className="py-2 px-4">{title.seqNo}</td>
      ...
      <div className="flex justify-between items-center mt-4 text-sm text-gray-600">
        <div>
          Showing 1 to {titles.length} of {titles.length} entries
        </div>
      </div>
    </div>
  );
