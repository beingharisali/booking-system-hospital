...
const ProvinceList: React.FC = () => {
  const provinces = [
    { sr: 1, code: '0092', country: 'Pakistan', province: 'Gilgit Baltistan' },
    { sr: 2, code: '0092', country: 'Pakistan', province: 'Capital' },
    { sr: 3, code: '0092', country: 'Pakistan', province: 'Balochistan' },
  ];
  ...
  return (
    <div className="p-4">
          ...
          <tbody className="text-gray-700 text-sm">
            {provinces.map((province, index) => (
              <tr key={index} className="border-b border-gray-300">
                <td className="py-2 px-4">{province.sr}</td>
                <td className="py-2 px-4">{province.code}</td>
                <td className="py-2 px-4">{province.country}</td>
                <td className="py-2 px-4">{province.province}</td>
      ...
      <div className="flex justify-between items-center mt-4 text-sm text-gray-600">
        <div>
          Showing 1 to {provinces.length} of {provinces.length} entries
        </div>
      </div>
    </div>
  );
